...
#[command(
    version,
    about,
    long_about = "Build tables from Kooima's thesis that are required for rendering planetary scale terrain."
)]
struct Opt {
...
impl Triangle {
    fn new(i0: u32, v0: IVec2, i1: u32, v1: IVec2, i2: u32, v2: IVec2) -> Self {
        Self {
            v: [v0, v1, v2],
            i: [i0, i1, i2],
        }
...
fn make_wireframe_list(max_level: usize) -> Result<()> {
    println!("// //////////////////////////////// AUTOGENERATED DEPS //////////////////////////////// //");
    println!("// cargo run -p dump-terrain-tables -- --dump-wireframe-list --max-level {max_level}");
...
fn make_triangle_strips(max_level: usize) -> Result<()> {
    println!("// //////////////////////////////// AUTOGENERATED DEPS //////////////////////////////// //");
    println!("// cargo run -p dump-terrain-tables -- --dump-triangle-strips");
    ...
    for subdivisions in 0..=max_level {
        ...
        let binned = bin_by_row(&tris);
        for &winding in &PatchWinding::all_windings() {
            let indices = make_triangle_strip(&bins, &binned, winding);
            let start = all_indices.len() as u32;
            all_indices.extend_from_slice(&indices);
            let end = all_indices.len() as u32;
            println!(
                "static TRI_STRIP_INDICES{}_{}_RANGE: std::ops::Range<u32> = {}..{};",
                subdivisions,
                winding.const_name(),
                start,
                end
    ...
    println!(
        "static TRI_STRIP_INDICES: [u32; {}] = {:?};",
        all_indices.len(),
        all_indices
    );
    println!("pub fn get_tri_strip_indices() -> &'static [u32] {{ &TRI_STRIP_INDICES }}");
    let mut func = String::new();
    func += "use crate::patch::patch_winding::PatchWinding;";
    func += "pub fn get_tri_strip_index_range(subdivisions: usize, winding: PatchWinding) -> std::ops::Range<u32> {\n";
    func += "    match subdivisions {\n";
    for subdivisions in 0..=max_level {
        writeln!(func, "        {subdivisions} => {{")?;
        func += "            match winding {\n";
        for &winding in &PatchWinding::all_windings() {
            writeln!(
                func,
                "                PatchWinding::{} => TRI_STRIP_INDICES{}_{}_RANGE.clone(),",
                winding.enum_name(),
                subdivisions,
                winding.const_name()
            )?;
...
fn make_index_dependency_luts(max_level: usize) -> Result<()> {
    println!("// //////////////////////////////// AUTOGENERATED DEPS //////////////////////////////// //");
    println!("// cargo run -p dump-terrain-tables -- --dump-index-dependency-luts");
    for i in 0..=max_level {
        let (_tris, deps) = make_tris(i);
        println!(
            "static INDEX_DEPENDENCY_LUT{}: [u32; {}] = [\n    {}\n];",
            i,
            deps.len() * 2,
            deps.iter()
                .map(|e| format!("{},\n    {}", e.a, e.b))
    ...
    let mut func = String::new();
    func += "pub fn get_index_dependency_lut(subdivisions: usize) -> &'static [u32] {\n";
    func += "    match subdivisions {\n";
    for subdivisions in 0..=max_level {
        writeln!(
            func,
            "        {subdivisions} => &INDEX_DEPENDENCY_LUT{subdivisions},"
        )?;
    }
    writeln!(
        func,
...
fn put_line_list_tri(tri: &Triangle, indices: &mut Vec<u32>) {
...
fn make_line_list(
    bins: &[(i32, i32)],
    binned: &HashMap<(i32, i32), Vec<Triangle>>,
    ...
    // base case
    if bins.len() == 1 {
        put_line_list_tri(binned[&bins[0]].first().unwrap(), &mut indices);
        return indices;
    }
    ...
    for pairs in bins.chunks(2).skip(1) {
        ...
        if winding.has_side0() {
            put_line_list_tri(tri_a, &mut indices);
            put_line_list_tri(tri_b, &mut indices);
        } else {
...
fn make_triangle_strip(
    ...
    // base case
    if bins.len() == 1 {
        let tri = binned[&bins[0]].first().unwrap();
        indices.push(tri.i[0]);
        indices.push(tri.i[1]);
        indices.push(tri.i[2]);
        return indices;
...
fn bin_by_row(tris: &[Triangle]) -> HashMap<(i32, i32), Vec<Triangle>> {
    let row_bins = collect_unique_rows(tris).drain(..).collect::<HashSet<_>>();
    let mut bins: HashMap<(i32, i32), Vec<Triangle>> = HashMap::new();
    for t in tris {
...
fn row_bin_for_tri(tri: &Triangle) -> (i32, i32) {
    let a = tri.v[0].y;
    let b = tri.v[1].y;
    let c = tri.v[2].y;
    if a == c {
...
fn collect_unique_rows(tris: &[Triangle]) -> Vec<(i32, i32)> {
    ...
    let mut v = uniq.drain().collect::<Vec<i32>>();
    v.sort_unstable();
...
fn make_tris(subdivisions: usize) -> (Vec<Triangle>, Vec<Edge>) {
    let s = i32::try_from(100 * (subdivisions + 1)).expect("subdivisions too big");
    let v0 = IVec2::new(0, s);
    let v1 = IVec2::new(-s, -s);
    let v2 = IVec2::new(s, -s);
...
fn get_index(
    ...
    v: &IVec2,
    edge: Edge,
    indices: &mut HashMap<IVec2, (u32, Edge)>,
