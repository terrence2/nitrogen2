...
impl<Unit> Default for Sphere<Unit>
where
    Unit: LengthUnit,
{
    fn default() -> Self {
        Self {
            center: Pt3::zero(),
            radius: Length::<Unit>::from(1_f64),
        }
    }
}

impl<Unit> Sphere<Unit>
where
    Unit: LengthUnit,
{
    pub fn from_center_and_radius(center: Pt3<Unit>, radius: Length<Unit>) -> Self {
        Self { center, radius }
    }
...
impl<Unit> RenderPrimitive for Sphere<Unit>
where
    Unit: LengthUnit,
{
    ...
    fn to_primitive(&self, detail: u32) -> Primitive {
        ...
        let s = 1f64;
        let mut init = vec![
            DVec3::new(-s, t, 0f64).normalize(),
            DVec3::new(s, t, 0f64).normalize(),
            DVec3::new(-s, -t, 0f64).normalize(),
            DVec3::new(s, -t, 0f64).normalize(),
            DVec3::new(0f64, -s, t).normalize(),
            DVec3::new(0f64, s, t).normalize(),
            DVec3::new(0f64, -s, -t).normalize(),
            DVec3::new(0f64, s, -t).normalize(),
            DVec3::new(t, 0f64, -s).normalize(),
            DVec3::new(t, 0f64, s).normalize(),
            DVec3::new(-t, 0f64, -s).normalize(),
            DVec3::new(-t, 0f64, s).normalize(),
        ];
        let mut verts = init
            .drain(..)
            .map(|position| Vertex::new_with_normal(position, position))
            .collect::<Vec<Vertex>>();
        ...
        for vert in &mut verts {
            vert.position = (self.center + (vert.position * self.radius)).dvec3();
        }
